...
/*
  bool isDiboson(std::string backgroundName);
  
  bool isZjets(std::string backgroundName);
*/
...
int main(){
  ...
  for(int i=0; i<plotLists::CUTSFORYIELDS.size(); i++){
    //    std::cout << "cuts for yields: " << plotLists::CUTSFORYIELDS[i] << std::endl;
    //    if(plotLists::CUTSFORYIELDS[i]=="_2_afterZpt__iso__2BIncl_2jets")
  ...
  for(int z=0; z<plotLists::SAMPLES_TO_PLOT.size(); z++){
	...
	background_name="$t\\bar{t}$";
      ...
      std::cout << "background name: " << background_name << std::endl;
    }
  }
  ...
  for(int bb=0; bb<systematicList.size(); bb++){
	...
	if(histograms[cc][dd]){
	  ...
	  valuesForTable[cc][bb][dd]=histograms[cc][dd]->IntegralAndError(1, numBins, error);
	  //std::cout << "values: " << valuesForTable[cc][aa][bb][dd] << std::endl;
	  if(bb==0)
	    mcErrors[cc][dd]=error;
	}else{ 
	  valuesForTable[cc][bb][dd]=0.;
	  if(bb==0)
	    mcErrors[cc][dd]=0.;
	}
  ...
  for (int d=0; d<plotNames.size(); d++){
    std::string outputFileName="output/tex/systematicTables/systematics_grouped"+plotLists::CUTSFORYIELDS[d];
  ...
  */
  ...
  for(int j=0; j<systList.size(); j++){
	...
	std::vector<double> comb_err=tools::getUpAndDownErrors(combinedValues[index], combinedValues[index+1], combinedValues[0], "");
	    ...
	    //	    std::cout << "nominal: " << combinedValues[0] << " combined values n: " << combinedValues[index] << " combined values n+1: " << combinedValues[index+1] << std::endl;
	    //	    std::cout << "totalUp: " << totalUp << " and totalDown: " << totalDown << " comb_err up: " << comb_err[0] << " comb_err down: " << comb_err[1] << std::endl;
      }
	...
	//	std::cout << "signal: " << ab << " nominal value: " << signalValues[0][ab] << " up error: " << up << std::endl;
